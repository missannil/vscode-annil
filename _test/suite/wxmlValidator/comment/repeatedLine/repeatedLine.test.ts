...
async function openFixtureAndWaitForInitialDiagnostics(uri: Uri): Promise<readonly Diagnostic[]> {
  ...
  return diagnosticsReady ?? waitForDiagnostics(uri, (current) => current.length === 0);
}

describe("annil 注释：repeatedLine", () => {
  let initialDiagnostics: readonly Diagnostic[];
  const uri = Uri.file(WXML_PATH);

  before(async () => {
    initialDiagnostics = await openFixtureAndWaitForInitialDiagnostics(uri);
  });

  test("插入重复 line 注释前不产生诊断", () => {
    assert.deepStrictEqual(initialDiagnostics, []);
  });

  test("插入第二个 line 注释后产生重复注释诊断", async () => {
    const edit = new WorkspaceEdit();
    edit.insert(uri, new Position(2, 0), `${LINE_COMMENT}\n${LINE_COMMENT}\n`);
