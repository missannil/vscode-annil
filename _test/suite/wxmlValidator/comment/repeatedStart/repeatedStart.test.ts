...
const WXML_PATH = path.join(projectRoot, "_test/suite/wxmlValidator/comment/repeatedStart/repeatedStart.wxml");
const START_COMMENT = "<!-- annil disable start -->";
const END_COMMENT = "<!-- annil disable end -->";

describe("annil 注释：repeatedStart", () => {
  ...
  test("插入重复 start 注释前不产生诊断", () => {
    assert.deepStrictEqual(initialDiagnostics, []);
  });

  test("插入第二个 start 注释后产生重复注释诊断", async () => {
    const edit = new WorkspaceEdit();
    edit.insert(uri, new Position(2, 0), `${START_COMMENT}\n${START_COMMENT}\n${END_COMMENT}\n`);
  ...
  after(async () => {
    const document = await workspace.openTextDocument(uri);
    if (document.lineAt(2).text !== START_COMMENT) return;

    const edit = new WorkspaceEdit();
    edit.delete(uri, new Range(new Position(2, 0), new Position(5, 0)));
