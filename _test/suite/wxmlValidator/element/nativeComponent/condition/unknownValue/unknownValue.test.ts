...
describe("conditionUnknownValue", () => {
  test("wx:if 和 wx:elif 的简单条件必须位于当前作用域", async () => {
    const wxmlUri = Uri.file(
      path.join(
        projectRoot,
        "_test/suite/wxmlValidator/element/nativeComponent/condition/unknownValue/unknownValue.wxml",
      ),
    );
