...
describe("camelCase", () => {
  test("未知属性产生诊断，合法属性不误报", async () => {
    const wxmlUri = Uri.file(
      path.join(
        projectRoot,
        "_test/suite/wxmlValidator/element/customComponent/unknownAttr/camelCase/camelCase.wxml",
      ),
    );
    ...
    assert.strictEqual(unknown[0].message, `未知属性: "unknownAttr"`);
    ...
    const unknownLine = document.getText().split("\n").findIndex((line) => line.includes("unknownAttr=\""));
    const unknownStart = document.lineAt(unknownLine).text.indexOf("unknownAttr");
    ...
    assert.strictEqual(unknown[0].range.end.character, unknownStart + "unknownAttr".length);
