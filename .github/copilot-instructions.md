...
- _test: 存放测试代码。
- **验证协作**：默认由用户在本地终端执行 `pnpm run compile:test`、当前聚焦的 `pnpm test:extension`，以及阶段性 `pnpm run check`；代理不主动在终端运行这些命令。
- **结果反馈**：验证通过时，用户只需回复“通过”；验证失败时，仅提供失败用例名、错误信息与相关堆栈片段。代理据此修正当前测试或实现。
- **代理验证边界**：代理保留必要的编辑器诊断检查，但不重复读取成功测试的完整日志，也不主动执行 Extension Host 测试、TypeScript 编译或全量静态检查。
- **测试前置确认**：准备新增或修改测试时，先以文字说明该测试的目的、所覆盖的诊断规则及其预期行为；在用户明确允许前，不得创建测试 fixture、测试文件或修改任何代码。
- **测试粒度**：以一条业务诊断规则为最小验证单元。该单元可包含诊断测试与其唯一对应的单项 Quick Fix 测试；不得混入其他规则、其他诊断或组件级 `fix-all` 行为。
- **诊断与修复协作**：诊断发布与单项 Quick Fix 应在同一规则目录中同步维护。实现上可将修复编辑生成到独立文件，但测试必须通过 Extension Host 请求真实 Code Action，不得直接调用修复生成函数；这样同时覆盖诊断、Provider 分发和 `WorkspaceEdit`。
...
- 新增或调整一个规则测试单元后，将其相对于 `_test/suite` 的诊断与单项 Quick Fix 测试路径加入 `_test/index.ts` 的 `manuallyFocusedTests`，且只启用当前待验证规则的路径。
- 完成当前规则测试单元后，等待用户运行 `pnpm test:extension` 并反馈结果；失败时仅修正当前规则的测试或实现，再等待用户重试。测试通过后，将该规则的路径在 `manuallyFocusedTests` 中注释掉但不删除，以保留执行记录；随后停止继续新增测试或修改实现，向用户报告结果并等待下一步指示。
...
- 重构逻辑后，等待用户运行 `pnpm test:extension` 并反馈结果；测试失败时修正相关实现或当前测试，测试通过后向用户报告结果。
