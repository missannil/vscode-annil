...
export class WxmlChecker {
  ...
  private currentCustomComponentList: string[] = [];
  // 记录元素节点的id,为了id重复检测
  ...
  private checkMissingComponentTag(): void {
    ...
    this.currentCustomComponentList.forEach((customComponentName) => {
  ...
  private removeCurrentCustomComponentList(element: Element): void {
    const tagName = element.name;
    const elementId: string | undefined = element.attribs.id;
    if (this.currentCustomComponentList.includes(elementId)) {
      const index = this.currentCustomComponentList.indexOf(elementId);
      if (index !== -1) {
        this.currentCustomComponentList.splice(index, 1);
      }
    } else {
      const index = this.currentCustomComponentList.indexOf(tagName);
      if (index !== -1) {
        this.currentCustomComponentList.splice(index, 1);
      }
    }
  ...
  private checkNodeList(
    ...
    for (const childNode of childNodeList) {
      ...
      if (nodeType.isElement(childNode)) {
        ...
        if (nodeType.isNativeTag(childNode)) {
